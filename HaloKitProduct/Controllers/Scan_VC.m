...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        [self pushVc:metadataObject.stringValue];
//        if ([self.delegate respondsToSelector:@selector(returnValue:)]) {
//            [self.delegate returnValue:metadataObject.stringValue];
//        }
    }
}
#pragma mark-> 我的相册
...
//            if ([self.delegate respondsToSelector:@selector(returnValue:)]) {
//                [self.delegate returnValue:scannedResult];
//            }
            [self pushVc:scannedResult];
...
- (void)pushVc:(NSString *)value{
    NSLog(@"valuevaluevalue:%@", value);
    //需要把返给我的字符串截取一部分
    
    NSString * str = [[self class] encryptWithContent:value type:kCCDecrypt key:@"hAlokiTs"];
    NSLog(@"strstrstr:%@", str);
    BGCodeInfoVC * CodeVc = [BGCodeInfoVC initVCWithParam:@"2" source:Source type:nil];
    [CodeVc getDeviceInfoWithCode:str finished:^(NSInteger code) {
        if (code == 1000017) {//无效的设备号
            [BGFailureVc push:self param:@"1" param1:nil];
        }else if (code == 2000019) {// 设备已经有人绑定过
            
            [BGFailureVc push:self param:@"2" param1:nil];
        }else if (code == 1) {
        
            [BGCodeInfoVC push:self param:str source: Source  type:nil];
...
- (void)getSource:(NSString *)source{
    Source = source;
    NSLog(@"self.sourc:::::%@", Source);
...
+(NSString *)encryptWithContent:(NSString *)content type:(CCOperation)type key:(NSString *)aKey
{
    ...
    const char *miChar;
    NSLog(@"contentCharcontentChar:%s", contentChar);

    miChar = encryptWithKeyAndType(contentChar, type, keyChar);
    NSLog(@"miCharmiCharmiChar:%s%@", miChar,[NSString stringWithCString:miChar encoding:NSUTF8StringEncoding] );
...
static const char* encryptWithKeyAndType(const char *text,CCOperation encryptOperation,char *key)
{
    ...
    NSLog(@"desStrdesStrdesStr:%@", desStr);

//    [userD setObject:desStr forKey:DESDEVICEID];
