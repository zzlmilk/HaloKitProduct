...
- (void)GET:(NSString *)URLString dict:(id)dict succeed:(void (^)(id data))succeed failure:(void (^)(NSError *error))failure
{
    ...
    [manager.responseSerializer setValue:token forKey:@"Access-Token"];
...
- (void)POSTHeader:(NSString *)URLString dict:(id)dict AndHeader:(NSString *)key  succeed:(void (^)(id data))succeed failure:(void (^)(NSError *error))failure
{
    //创建网络请求管理对象
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    //申明返回的结果是json类型
    ...
    [manager.requestSerializer setValue:key forHTTPHeaderField:@"Access-Token"];
    //发送网络请求(请求方式为POST)
    [manager POST:URLString parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)GETHeader:(NSString *)URLString dict:(id)dict  AndHeader:(NSString *)key  succeed:(void (^)(id data))succeed failure:(void (^)(NSError *error))failure
{
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
